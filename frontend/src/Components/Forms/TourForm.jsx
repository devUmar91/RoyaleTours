...
const TourForm = () => {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    try {
      const response = await axios.post("http://127.0.0.1:4000/api/tour/add", {
        type,
  ...
  <form className=''>
      <div className="relative z-0 w-full mb-6 group">
        ...
        onChange={(e)=>setTourname(e.target.value)}
          ...
          name="tour_name"
          id="tour_name"
          className="block py-2.5 px-0 text-gray-900 w-full text-sm  bg-transparent border-0 border-b-2 border-gray-300 appearance-none dark:border-gray-600 dark:focus:border-blue-500 focus:outline-none focus:ring-0 focus:border-blue-600 peer"
          placeholder=" "
          required
        />
        <label
